...
def show():
    ...
    if wanted is None:
        wanted = ''
    rs = list(Nodes.query.filter(Nodes.name.like('%' + wanted + '%')).all())
    return render_template('force.html',nodesdata=nodesdata, nodesnum=nodesnum, edgesdata=edgesdata, edgesnum=edgesnum,rs=rs)
